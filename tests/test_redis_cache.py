...
class TestRedisConnection:
    ...
    async def test_ping_redis_function(self):
        ...
        Patches settings.redis_url to TEST_REDIS_URL so ping_redis() connects
        to the test instance, not whatever REDIS_URL points to in the environment.
        """
        with patch.object(settings, "redis_url", TEST_REDIS_URL):
...
class TestCacheStatistics:
    ...
    @pytest.mark.asyncio
    async def test_get_cache_stats_default(self, token_cache):
        """Test that default stats (no token count) return memory info only."""
    ...
    @pytest.mark.asyncio
    async def test_get_cache_stats_with_token_count(self, token_cache):
        """Test stats with include_token_count=True returns cached_tokens."""
        for i in range(5):
            await token_cache.set_token(f"user_{i}", f"token_{i}", 3600)

        stats = await token_cache.get_cache_stats(include_token_count=True)
...
class TestKeyGeneration:
    ...
    async def test_sanitization_in_lock_key(self, token_cache):
        with pytest.raises(ValueError, match="invalid characters"):
            token_cache._get_lock_key("test:user")
