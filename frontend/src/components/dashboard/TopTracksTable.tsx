...
const TrackRow = ({ track, index }: { track: Track; index: number }) => (
  <tr className="group hover:bg-white/[0.02] transition-colors border-b border-white/[0.02] last:border-0">
    <td className="py-3 text-outline font-bold w-8">{index + 1}</td>
    <td className="py-3">
...
export const TopTracksTable = () => {
  ...
  // Extract state-driven render decision out of nested ternary.
  const renderBody = () => {
    if (isTopItemsFetching && !topItemsData) {
      return <SkeletonRows />;
    }
    if (!topItemsData?.items?.length) {
      return <EmptyRow />;
    }
    return topItemsData.items.map((track: Track, i: number) => (
    ...
    <div className="glass-card rounded-3xl p-6 sm:p-8 flex flex-col">
      ...
      <div className="flex-1 overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="text-sm">{renderBody()}</tbody>
        </table>
      </div>
